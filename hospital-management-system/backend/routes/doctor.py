...
def create_doctor(doctor: schemas.DoctorCreate, db: Database = Depends(get_db)):
    return crud.create_doctor(db=db, doctor=doctor)

@router.get("/", response_model=List[schemas.Doctor])
def read_doctors(skip: int = 0, limit: int = 100, db: Database = Depends(get_db)):
    doctors = crud.get_doctors(db, skip=skip, limit=limit)
    return doctors

@router.get("/{doctor_id}", response_model=schemas.Doctor)
def read_doctor(doctor_id: str, db: Database = Depends(get_db)):
...
def update_doctor(doctor_id: str, doctor: schemas.DoctorCreate, db: Database = Depends(get_db)):
...
def delete_doctor(doctor_id: str, db: Database = Depends(get_db)):
