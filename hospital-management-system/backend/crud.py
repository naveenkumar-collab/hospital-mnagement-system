...
def update_patient(db: Database, patient_id: str, patient: schemas.PatientCreate):
    db.patients.update_one({"_id": ObjectId(patient_id)}, {"$set": serialize(patient)})
...
def update_doctor(db: Database, doctor_id: str, doctor: schemas.DoctorCreate):
    db.doctors.update_one({"_id": ObjectId(doctor_id)}, {"$set": serialize(doctor)})
...
def get_appointments(db: Database, skip: int = 0, limit: int = 100):
    return [map_appointment(doc) for doc in db.appointments.find().skip(skip).limit(limit)]
...
def update_appointment(db: Database, appointment_id: str, appointment: schemas.AppointmentCreate):
    db.appointments.update_one({"_id": ObjectId(appointment_id)}, {"$set": serialize(appointment)})
